...
PetscErrorCode MPIRead_Blocking(FILE *fp,void **data,long int len,size_t size,int root,PetscBool skip_header,MPI_Comm comm)
{
  ...
  if (rank == root) {
    if (!skip_header) {
      /* read header */
      if (fread(&len_total_bytes,sizeof(long int),1,fp) != 1) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_FILE_READ,"fread() error");
  ...
  if (rank == root) {
    ...
    for (r=0; r<commsize; r++) {
      ...
      if (r == root) {
        ...
        if (fread(buffer,size,ipackr[1],fp) != (size_t) ipackr[1]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_FILE_READ,"fread() error");

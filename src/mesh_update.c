...
PetscErrorCode UpdateMeshGeometry_ApplyDiffusionJMAX_BaseLevel(DM dav, 
  PetscReal *diffusivities, 
  PetscReal baselvl,
  PetscReal timespan,
  PetscBool dirichlet_east,
  PetscBool dirichlet_west,
  PetscBool dirichlet_front,
  PetscBool dirichlet_back,
  PetscBool only_update_surface)
{
  PetscErrorCode ierr;
  DM daH,cda;
  PetscInt nsteps,s,nM;
  ...
  PetscInt i,j,k,e,nel,nen,MX,MY,MZ;
  const PetscInt *elnidx;
  PetscInt ge_eqnums[Q1_NODES_PER_EL_3D];
  PetscScalar el_coords[NSD*Q1_NODES_PER_EL_3D];
  PetscScalar el_h[Q1_NODES_PER_EL_3D];
  PetscScalar ***LA_H3,***LA_Hinit3;
  PetscInt NI,NJ,NK,si,sj,sk,ni,nj,nk;
  PetscReal MeshMin[3],MeshMax[3],ds;
  PetscBool view_surface_diffusion = PETSC_FALSE;
  ...
  /* compute a time step based on x,z cell sizes and a CFL of 0.5 */
  dt_explicit = 0.1 *(ds*ds) / PetscMax(diffusivities[0],diffusivities[1]);
  PetscPrintf(PETSC_COMM_WORLD,"  [time step] dt_explicit = %1.4e \n",dt_explicit);
  ...
  for (s=0; s<nsteps; s++) {
    ...
    for (e=0; e<nel; e++) {
      ...
      height = 0.0;
      for (i=0; i<4; i++) {
        height += Ni[i] * el_h2d[i];
      }
      /* set diffusivity based on height */
      if (height < baselvl) {
        diffusivity = diffusivities[0];
      } else {
        diffusivity = diffusivities[1];
      }

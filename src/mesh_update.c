...
PetscErrorCode UpdateMeshGeometry_ApplyDiffusionJMAX_Spherical(DM dav,PetscReal diffusivity,PetscReal timespan,
                                                               PetscBool dirichlet_east,PetscBool dirichlet_west,PetscBool dirichlet_front,PetscBool dirichlet_back,PetscBool only_update_surface)
{
  PetscErrorCode ierr;
  DM daH,cda;
  ...
  PetscReal dt_explicit;
  Vec Hinit,H,local_H,coords,gcoords,diagM,rhs,local_rhs,local_M;
  PetscScalar *LA_H,*LA_gcoords,*LA_coords,*LA_rhs,*LA_M;
  PetscInt i,j,k,e,d,nel,nen,MX,MY,MZ,nidx;
  const PetscInt *elnidx;
  PetscInt ge_eqnums[NODES_PER_EL_Q1_3D];
  PetscScalar el_coords[NSD*NODES_PER_EL_Q1_3D];
  PetscScalar el_h[NODES_PER_EL_Q1_3D];
  ...
  for (k=0; k<nk; k++) {
    for (j=0; j<nj; j++){
      for (i=0; i<ni; i++) {
        ...
        elevation = 0.0;
        for (d=0; d<3; d++) {
        elevation += LA_coords[3*nidx + d] * LA_coords[3*nidx + d];
        }
        /* Set height */
        LA_H[nidx] = PetscSqrtReal(elevation);
      }
    }
  }
  ierr = VecRestoreArray(coords,&LA_coords);CHKERRQ(ierr);
  ierr = VecRestoreArray(H,&LA_H);CHKERRQ(ierr);
  /* Copy H into H_init */
  ierr = VecCopy(H,Hinit);CHKERRQ(ierr);
...

  {
    ...
    if (only_update_surface) {
      ...
      if ( NJv == sjv+njv ) {
        ...
        for (k=skv; k<skv+nkv; k++) {
          for (i=siv; i<siv+niv; i++) {
            PetscReal coor_3d[3],A,theta,phi,elevation;
            ...
            coor_3d[0] = LA_coords3[k][j][i].x;
            coor_3d[1] = LA_coords3[k][j][i].y;
            coor_3d[2] = LA_coords3[k][j][i].z;
            ...
            elevation = LA_H3[k/2][j/2][i/2];

            /* Transform back in cartesian coordinates */
            LA_coords3[k][j][i].x = elevation * PetscCosReal(theta);
            LA_coords3[k][j][i].y = elevation * PetscSinReal(theta) * PetscCosReal(phi) ; 
            LA_coords3[k][j][i].z = elevation * PetscSinReal(theta) * PetscSinReal(phi);

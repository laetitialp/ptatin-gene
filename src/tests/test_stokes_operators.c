...
PetscErrorCode perform_viscous_solve(PhysCompStokes user)
{
  ...
  ierr = PetscViewerASCIIOpen(PetscObjectComm((PetscObject)ksp),NULL,&monviewer);CHKERRQ(ierr);
  {
    ...
    ierr = KSPMonitorSet(ksp,(PetscErrorCode (*)(KSP,PetscInt,PetscReal,void*))KSPMonitorResidualShort,vf,(PetscErrorCode (*)(void**))PetscViewerAndFormatDestroy);CHKERRQ(ierr);
  }
  ...
  PetscPrintf(PETSC_COMM_WORLD,"KSPSolveA11(its = %" PetscInt_FMT ",cycles = %" PetscInt_FMT ")   time %1.4e (sec): ratio %1.4e%%: min/max %1.4e %1.4e (sec)\n",its,iterations,tl,100.0*(timeMIN/timeMAX),timeMIN,timeMAX);

...
typedef enum { OP_TYPE_REDISC_ASM=0, OP_TYPE_REDISC_MF, OP_TYPE_GALERKIN, OP_TYPE_MFGALERKIN } OperatorType;//, OP_TYPE_MFGALERKIN, OP_TYPE_SNESFD } OperatorType;
...
PetscErrorCode MatAssembleMFGalerkin(DM dav_fine,BCList u_bclist_fine,Quadrature volQ_fine,DM dav_coarse,Mat Ac)
{
...
#if 0
  {
    ...
    ierr = SurfBCList_AssembleAij(surf_bclist,ij,dav_coarse,NULL,Ac);CHKERRQ(ierr);
    if (surf_bclist->sc_nreg > 0) {
      ierr = MatZeroRows_BCList(Ac,u_bclist,dav_coarse,NULL);CHKERRQ(ierr);
      ierr = MatZeroCols_BCList(Ac,u_bclist,dav_coarse,NULL);CHKERRQ(ierr);
    }
  }
#endif
  ...
  /* 
  refi, refj, refk are the refinement factors between dav_fine and dav_coarse
  why is it written Q2 cell problem contains: [refi x refj x refk] elements ?
  */
  ...
  PetscPrintf(PETSC_COMM_WORLD,"  Q2 cell problem contains: [%" PetscInt_FMT " x %" PetscInt_FMT " x %" PetscInt_FMT "] elements, [%" PetscInt_FMT " x %" PetscInt_FMT " x %" PetscInt_FMT "] nodes\n",refi,refj,refk,nni,nnj,nnk);
  ...
  if (sei%refi != 0) { SETERRQ(PETSC_COMM_SELF,PETSC_ERR_USER,"Start element (i) must be divisible by %" PetscInt_FMT "",refi); }
  if (sej%refj != 0) { SETERRQ(PETSC_COMM_SELF,PETSC_ERR_USER,"Start element (j) must be divisible by %" PetscInt_FMT "",refj); }
  if (sek%refk != 0) { SETERRQ(PETSC_COMM_SELF,PETSC_ERR_USER,"Start element (k) must be divisible by %" PetscInt_FMT "",refk); }
  
  if (lmi%refi != 0) { SETERRQ(PETSC_COMM_SELF,PETSC_ERR_USER,"Local element size (i) must be divisible by %" PetscInt_FMT "",refi); }
  if (lmj%refj != 0) { SETERRQ(PETSC_COMM_SELF,PETSC_ERR_USER,"Local element size (j) must be divisible by %" PetscInt_FMT "",refj); }
  if (lmk%refk != 0) { SETERRQ(PETSC_COMM_SELF,PETSC_ERR_USER,"Local element size (k) must be divisible by %" PetscInt_FMT "",refk); }
  ...
  // compute P^T * Acell * P, Acell is non zero, so what is inside ? What does Ac_el contain ?
  ...
  PetscTime(&t[0]);
  for (kc=0; kc<lmk/refk; kc++) { // why not directly loop over lmk_coarse ?
    for (jc=0; jc<lmj/refj; jc++) {
      for (ic=0; ic<lmi/refi; ic++) {
        ...
        // if Ac_el is zeroed why compute P^T * Acell * P before ?	
  ...
  for (k=nlevels-1; k>=0; k--) {
    ...
    switch (level_type[k]) {
      ...
      case OP_TYPE_MFGALERKIN:
      {
        ...
        if (!been_here) PetscPrintf(PETSC_COMM_WORLD,"Level [%" PetscInt_FMT "]: Coarse grid type :: MFGalerkin :: assembled operator \n", k);
        if (k == nlevels-1) {
        ...
        if (level_type[k+1] != OP_TYPE_REDISC_MF) {
          SETERRQ(PETSC_COMM_WORLD,PETSC_ERR_ARG_OUTOFRANGE,"Cannot use mf-galerkin. Next finest level[%" PetscInt_FMT "] must be of type OP_TYPE_REDISC_MF",k+1);
        }
...
PetscErrorCode HMG_SetUp(AuuMultiLevelCtx *mlctx, pTatinCtx user)
{
  ...
  if (nlevels >= MAX_MG_LEVELS) SETERRQ(PETSC_COMM_WORLD,PETSC_ERR_SUP,"Maximum number of multi-grid levels is set by #define MAX_MG_LEVELS %" PetscInt_FMT "",MAX_MG_LEVELS);
  ...
  for (k=nlevels-1; k>=0; k--) {
    ...
    ierr = PetscSNPrintf(name,PETSC_MAX_PATH_LEN-1,"vel_dmda_Lv%" PetscInt_FMT "",k);CHKERRQ(ierr);
  ...
  */
  ...
  ierr = PetscMalloc1(nlevels,&mlctx->surfQ);CHKERRQ(ierr);
  ierr = PetscMalloc1(nlevels,&mlctx->surf_bclist);CHKERRQ(ierr);
  ierr = PetscMalloc1(nlevels,&mlctx->mfi);CHKERRQ(ierr);  
  ...
  for (k=0; k<nlevels; k++) {
    ...
    mlctx->surf_bclist[k]         = NULL;
    mlctx->surfQ[k]               = NULL;
    mlctx->mfi[k]                 = NULL;

    mlctx->level_type[k]          = OP_TYPE_REDISC_MF;
  ...
  for (k=0; k<nlevels; k++) {
    ...
    mlctx->surf_bclist[k]         = surf_bclist[k];
    mlctx->surfQ[k]               = surfQ[k];
    mlctx->mfi[k]                 = mfi[k];
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode HMG_Destroy(AuuMultiLevelCtx *mlctx)
{
  ...
  for (k=0; k<mlctx->nlevels-1; k++) {
    ierr = MeshFacetInfoDestroy(&mlctx->mfi[k]);CHKERRQ(ierr);
    ierr = SurfaceQuadratureDestroy(&mlctx->surfQ[k]);CHKERRQ(ierr);
  }
  for (k=0; k<mlctx->nlevels-1; k++) {
    ierr = BCListDestroy(&mlctx->u_bclist[k]);CHKERRQ(ierr);
    ierr = SurfBCListDestroy(&mlctx->surf_bclist[k]);CHKERRQ(ierr);
  ...
  ierr = PetscFree(mlctx->surf_bclist);CHKERRQ(ierr);
...
PetscErrorCode GenerateICStateFromModelDefinition_FV(pTatinCtx *pctx)
{
 ...
 /* Generate physics modules */
  ...
  
  {
    ...
    ierr = PetscSNPrintf(prefix,PETSC_MAX_PATH_LEN-1,"step%" PetscInt_FMT "",user->step);CHKERRQ(ierr);
...
PetscErrorCode Run_NonLinearFV(pTatinCtx user,Vec v1,Vec v2)
{
  ...
  for (step=step0; step<=user->nsteps; step++) {
    ...
    /* update markers = >> gauss points */
    {
      ...
      ierr = SwarmUpdateGaussPropertiesLocalL2Projection_Q1_MPntPStokes_Hierarchy(user->coefficient_projection_type,npoints,mp_std,mp_stokes,mgctx.nlevels,mgctx.interpolation_eta,mgctx.dav_hierarchy,mgctx.volQ,mgctx.surfQ,mgctx.mfi);CHKERRQ(ierr);
    ...
    ierr = pTatinModel_ApplyBoundaryCondition(model,user);CHKERRQ(ierr);
    /* Coarse grid setup: Configure boundary conditions */
    ierr = pTatinModel_ApplyBoundaryConditionMG(mgctx.nlevels,mgctx.u_bclist,mgctx.surf_bclist,mgctx.dav_hierarchy,model,user);CHKERRQ(ierr);

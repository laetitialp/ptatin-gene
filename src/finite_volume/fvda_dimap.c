...
PetscErrorCode DIMapApply(DIMap map,PetscInt i,PetscInt *j)
{
  ...
  if (i < map->input_range[0]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i %" PetscInt_FMT " outside range [%" PetscInt_FMT ",%" PetscInt_FMT ")",i,map->input_range[0],map->input_range[1]);
  if (i >= map->input_range[1]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i %" PetscInt_FMT " outside range [%" PetscInt_FMT ",%" PetscInt_FMT ")",i,map->input_range[0],map->input_range[1]);
#endif

  *j = map->idx[i];
  
  if ((*j < 0) && (!map->negative_output_allowed)) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i %" PetscInt_FMT " maps to negative value %" PetscInt_FMT " : DIMap config does not allow negative output",i,*j);
...
PetscErrorCode DIMapApplyN(DIMap map,PetscInt N,PetscInt i[],PetscInt j[])
{
...
#if defined(PETSC_USE_DEBUG)
        if (i[k] < map->input_range[0]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i %" PetscInt_FMT " outside range [%" PetscInt_FMT ",%" PetscInt_FMT ")",i[k],map->input_range[0],map->input_range[1]);
        
        if (i[k] >= map->input_range[1]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i %" PetscInt_FMT " outside range [%" PetscInt_FMT ",%" PetscInt_FMT ")",i[k],map->input_range[0],map->input_range[1]);
#endif
      }
    }
  } else {
#if defined(PETSC_USE_DEBUG)
    for (k=0; k<N; k++) {
      if (i[k] < map->input_range[0]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i[%" PetscInt_FMT "] %" PetscInt_FMT " outside range [%" PetscInt_FMT ",%" PetscInt_FMT ")",k,i[k],map->input_range[0],map->input_range[1]);
      
      if (i[k] >= map->input_range[1]) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i[%" PetscInt_FMT "] %" PetscInt_FMT " outside range [%" PetscInt_FMT ",%" PetscInt_FMT ")",k,i[k],map->input_range[0],map->input_range[1]);
    }
#endif
  }
  ...
  if (!map->negative_output_allowed) {
    if (j[k] < 0) SETERRQ(PETSC_COMM_SELF,PETSC_ERR_SUP,"i[%" PetscInt_FMT "] %" PetscInt_FMT " maps to negative value %" PetscInt_FMT " : DIMap config does not allow negative output",k,i[k],j[k]);
  }

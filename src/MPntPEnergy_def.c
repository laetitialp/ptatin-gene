/*
  ...
  on otsu.local, at 2024-11-14 11:51:29.335611 by dmay
*/
...
const int MPntPEnergy_nmembers = 3;
...
void MPntPEnergyView(MPntPEnergy *point)
{
  ...
  {
    double data;
    MPntPEnergyGetField_heat_source_init(point,&data);
    printf("field: heat_source_init = %1.6e; [size %zu; type double; variable_name heat_source_init]\n",data, MPntPEnergy_member_sizes[2] );
  }
}
...
void MPntPEnergyVTKWriteAsciiAllFields(FILE *vtk_fp,const int N,const MPntPEnergy points[]) 
{
  ...
  fprintf( vtk_fp, "\t\t\t\t</DataArray>\n");
  fprintf( vtk_fp, "\t\t\t\t<DataArray type=\"Float64\" Name=\"heat_source_init\" format=\"ascii\">\n");
  for(p=0;p<N;p++) {
    fprintf( vtk_fp,"\t\t\t\t\t%lf\n",(double)points[p].heat_source_init);
  }
  fprintf( vtk_fp, "\t\t\t\t</DataArray>\n");
}
...
void MPntPEnergyPVTUWriteAllPPointDataFields(FILE *vtk_fp) 
{
  ...
  fprintf(vtk_fp, "\t\t\t<PDataArray type=\"Float64\" Name=\"heat_source_init\" NumberOfComponents=\"1\"/>\n");
}
...
void MPntPEnergyVTKWriteBinaryAppendedHeaderAllFields(FILE *vtk_fp,int *offset,const int N,const MPntPEnergy points[]) 
{
  ...
  *offset = *offset + sizeof(int) + N * sizeof(double);
  ...
  fprintf( vtk_fp, "\t\t\t\t<DataArray type=\"Float64\" Name=\"heat_source_init\" format=\"appended\"  offset=\"%d\" />\n",*offset);
  *offset = *offset + sizeof(int) + N * sizeof(double);
...
void MPntPEnergyVTKWriteBinaryAppendedDataAllFields(FILE *vtk_fp,const int N,const MPntPEnergy points[]) 
{
  ...
  fwrite( &length,sizeof(int),1,vtk_fp);
  for(p=0;p<N;p++) {
    fwrite( &points[p].heat_source_init,atomic_size,1,vtk_fp);
  }
...
int MPntPEnergyCreateMPIDataType(MPI_Datatype *ptype)
{
  ...
  memset(&dummy,0,sizeof(MPntPEnergy));
  ierr = MPI_Get_address(&dummy,&loc[0]);
  ierr = MPI_Get_address(&dummy.diffusivity,&loc[1]);
  ierr = MPI_Get_address(&dummy.heat_source,&loc[2]);
  ierr = MPI_Get_address(&dummy.heat_source_init,&loc[3]);

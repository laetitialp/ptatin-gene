...
PetscErrorCode FormFunctionLocal_U_FSSA(PhysCompStokes user,DM dau,PetscReal theta,PetscReal dt,const PetscScalar ufield[],PetscScalar Ru[])
{
  ...
  for (edge=0; edge<HEX_EDGES; edge++) {
    ...
    /* Only apply the FFSA on the JMAX face */
    if (edge != 2) {
      continue;
    }
...
PetscErrorCode FormFunction_StokesU(SNES snes,Vec X,Vec F,void *ctx)
{
  ...
  ierr = VecZeroEntries(F);CHKERRQ(ierr);
  //ierr = DMCompositeGather(stokes_pack,ADD_VALUES,F,FUloc,FPloc);CHKERRQ(ierr);
  ierr = DMLocalToGlobalBegin(dau,FUloc,ADD_VALUES,F);CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd  (dau,FUloc,ADD_VALUES,F);CHKERRQ(ierr);
  ...
  //ierr = BCListResidualDirichlet(stokes->u_bclist,u,Fu);CHKERRQ(ierr);
  ierr = BCListResidualDirichlet(stokes->u_bclist,X,F);CHKERRQ(ierr);

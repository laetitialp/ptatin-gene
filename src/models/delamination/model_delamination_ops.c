...
PetscErrorCode ModelAdaptMaterialPointResolution_Delamination(pTatinCtx c,void *ctx)
{
  PetscErrorCode ierr;
  PetscFunctionBegin;
  PetscPrintf(PETSC_COMM_WORLD,"[[%s]]\n", PETSC_FUNCTION_NAME);
...
PetscErrorCode ModelApplyGravity_Delamination(pTatinCtx c, void *ctx)
{
  Gravity        gravity;
...
PetscErrorCode pTatinModelRegister_Delamination(void)
{
  ...
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_ADAPT_MP_RESOLUTION,   (void (*)(void))ModelAdaptMaterialPointResolution_Delamination);CHKERRQ(ierr);
  ...
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_GRAVITY,         (void (*)(void))ModelApplyGravity_Delamination);CHKERRQ(ierr);

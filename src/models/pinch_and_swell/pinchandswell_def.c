...
PetscErrorCode ModelInitialize_PAS(pTatinCtx ptatinctx,void *modelctx)
{
  ...
  modeldata->domain[2] = 1.0;
  {
    ...
    if (flg) {
      if (nd != 3) {
        SETERRQ(PETSC_COMM_WORLD,PETSC_ERR_USER,"param:\"model domain size\" - status:\"missing entry\" => expected 3 entries, only found %" PetscInt_FMT "",nd);
      }
    }
  }
...
PetscErrorCode ModelApplyBoundaryConditionMG_PAS(PetscInt nl,BCList bclist[],SurfBCList surf_bclist[],DM dav[],pTatinCtx ptatinctx,void *modelctx)
{
  ModelCtx         *modeldata = (ModelCtx*)modelctx;
  PetscInt         n;
  PetscErrorCode   ierr;

...
PetscErrorCode ModelApplyBoundaryConditionMG_Rift_oblique3d(PetscInt nl,BCList bclist[],SurfBCList surf_bclist[],DM dav[],pTatinCtx user,void *ctx)
{
  ModelRift_oblique3dCtx *data = (ModelRift_oblique3dCtx*)ctx;
  PetscInt       n;
  PetscErrorCode ierr;
...
PetscErrorCode ModelApplyGravity_Rift_oblique3d(pTatinCtx c, void *ctx)
{
  Gravity        gravity;
  PetscReal      gvec[] = { 0.0, -10.0, 0.0 };
  PetscErrorCode ierr;
...
PetscErrorCode ModelApplyUpdateMeshGeometry_Rift_oblique3d_semi_eulerian(pTatinCtx c,Vec X,void *ctx)
{
  ...
  /* SURFACE REMESHING */
  ierr = UpdateMeshGeometry_FullLag_ResampleJMax_RemeshJMIN2JMAX(dav,velocity,NULL,step,PETSC_FALSE);
  ierr = DMCompositeRestoreAccess(stokes_pack,X,&velocity,&pressure);CHKERRQ(ierr);
...
PetscErrorCode pTatinModelRegister_Rift_oblique3d(void)
{
  ...
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_GRAVITY,         (void (*)(void))ModelApplyGravity_Rift_oblique3d);CHKERRQ(ierr);

...
PetscErrorCode ModelApplyBoundaryConditionMG_StaticBoxTM(PetscInt nl,BCList bclist[],SurfBCList surf_bclist[],DM dav[],pTatinCtx c,void *ctx)
{
  PetscScalar    zero = 0.0;
  PetscInt       n;
  PetscErrorCode ierr;
...
PetscErrorCode ModelApplyGravity_StaticBoxTM(pTatinCtx c, void *ctx)
{
  Gravity        gravity;
  PetscReal      gvec[] = { 0.0, -10.0, 0.0 };
  PetscErrorCode ierr;
...
PetscErrorCode pTatinModelCreate_StaticBoxTM(pTatinModel m)
{
  ...
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_GRAVITY,         (void (*)(void))ModelApplyGravity_StaticBoxTM);CHKERRQ(ierr);

...
static PetscErrorCode ComputeBallVelocity1(pTatinCtx ptatin, Vec X, ModelStokesLawCtx *data)
{
  PhysCompStokes    stokes;
  DM                stokes_pack,dau,dap;
  Vec               Uloc,Ploc;
  ...
  /*
  We don't know how much markers compose the ball so we
  allocate for the whole number of markers
  */
  ierr = PetscMalloc1(n_mp_points,&sphere_pidx);CHKERRQ(ierr);

  /* First we count how much markers compose the ball */
  cnt_ball_point = 0;
  for (p=0; p<n_mp_points; p++) {
    MPntStd  *mpp_std;
    int      region;
    ...
    /* Access region of the marker */
    MPntStdGetField_phase_index(mpp_std,&region);
    /* Access pidx of the marker */
    MPntStdGetField_point_index(mpp_std,&pidx);

    /* Ball's region index is 1 */
    if (region == 1) { 
      sphere_pidx[cnt_ball_point] = pidx;
      cnt_ball_point++;
    }
  }

  u_avg[0] = u_avg[1] = u_avg[2] = 0.0;
  for (p=0; p<cnt_ball_point; p++) {
    MPntStd     *mpp_std;
    double      *xi;
    int         eidx,point_idx;

    /* Get the point index from sphere_pidx */
    point_idx = sphere_pidx[p];

    DataFieldAccessPoint(PField_std,point_idx,(void**)&mpp_std);   

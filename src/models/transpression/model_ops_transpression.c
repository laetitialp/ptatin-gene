...
/* Developed by Anthony Jourdon [jourdon.anthon@gmail.com] for rift2subduction */
/* Modified for transpression by Josselin Gremmel and Guillaume Duclaux */
...
static PetscErrorCode ModelSetMaterialParameters(pTatinCtx c, ModelTranspressionCtx *data);
PetscErrorCode SwarmMPntStd_CoordAssignment_FaceLatticeLayout3d_epsilon_transpression(DM da,PetscInt Nxp[],PetscReal perturb, PetscReal epsilon, PetscInt face_idx,DataBucket db);

PetscErrorCode ModelInitialize_Transpression(pTatinCtx c,void *ctx)
{
  ...
  data->y_continent[0] = -20.0e3; // Conrad
  data->y_continent[1] = -60.0e3; // Moho
  data->y_continent[2] = -100.0e3; // LAB

  data->weak_base = -70.0e3; //base of the weak zone
  data->beta = 45; //weak zone obliquity
  
  data->wz = 100.0e3; // weak zone width
  
  /* Velocity */
  /* Orthogonal compression */
  data->v_compression = 1.0;
  /* Timing for BCs change */
  data->BC_time[0] = 100.0; //we only run the model for 30 Ma, so the bnext BC_time are not used
  data->BC_time[1] = 110.0;
  data->BC_time[2] = 210.0;
  data->BC_time[3] = 220.0;
  /* Oblique Compression */
  data->normV = 1.0;
  ...
  ierr = PetscOptionsGetReal(NULL,MODEL_NAME_RS,"-v_compression",  &data->v_compression,NULL);CHKERRQ(ierr);
  ...
  ierr = PetscOptionsGetReal(NULL,MODEL_NAME_RS,"-weak_base",      &data->weak_base,NULL);CHKERRQ(ierr);
  ierr = PetscOptionsGetReal(NULL,MODEL_NAME_RS,"-beta",      &data->beta,NULL);CHKERRQ(ierr);
  nn = 10;
  ...
  data->open_base       = PETSC_FALSE; //Pressure at the base could be impletemed based on this
  ...
  data->weak_base      = data->weak_base / data->length_bar;
  data->beta           = M_PI * data->beta / 180.0;
  /* Scale velocity */
  data->v_compression = data->v_compression*cm_per_yer2m_per_sec/data->velocity_bar;
  ...
  PetscPrintf(PETSC_COMM_WORLD,"[transpression]:  during the solve scaling is done using \n");
...
PetscErrorCode ModelApplyInitialMeshGeometry_Transpression(pTatinCtx c,void *ctx)
{
  ModelTranspressionCtx *data = (ModelTranspressionCtx*)ctx;
...
PetscErrorCode ModelApplyInitialMaterialGeometry_Transpression(pTatinCtx c,void *ctx)
{
  ModelTranspressionCtx *data = (ModelTranspressionCtx*)ctx;
  DataBucket                db;
  ...
  for (p=0; p<n_mp_points; p++) {
    ...
    if (position[1] >= data->weak_base) {
      if ((position[0] >=  (0.5 * data->Lz - position[2])*tan(data->beta) + (0.5 * data->Lx - 0.5*data->wz/cos(data->beta))) &&
                  ...
                  pls = ptatin_RandomNumberGetDouble(0.0,0.2);
      }
    }
...
PetscErrorCode ModelApplyInitialSolution_Transpression(pTatinCtx c,Vec X,void *ctx)
{
  ModelTranspressionCtx                        *data;
  ...
  vxl =  data->v_compression;
  vxr =  -data->v_compression;
  vzl = 0.0;
  vzr = 0.0;
...
PetscErrorCode ModelApplyInitialStokesVariableMarkers_Transpression(pTatinCtx c,Vec X,void *ctx)
{
  DM                         stokes_pack,dau,dap;
  PhysCompStokes             stokes;
  Vec                        Uloc,Ploc;
...
PetscBool BCListEvaluator_Lithosphere_transpression(PetscScalar position[],PetscScalar *value,void *ctx)
{
...
static PetscErrorCode ModelApplyBoundaryCondition_OrthogonalCompression(BCList bclist,DM dav,pTatinCtx c,ModelTranspressionCtx *data)
{
  ...
  bcdata->y_lab = data->Oy;
  bcdata->v = vxl;
  //ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMIN_LOC,0,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMIN_LOC,0,BCListEvaluator_constant,(void*)&vxr);CHKERRQ(ierr);
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMIN_LOC,1,BCListEvaluator_constant,(void*)&zero);CHKERRQ(ierr);
  
  bcdata->y_lab = data->Oy;
  bcdata->v = vxr;
  //ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMAX_LOC,0,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
...
static PetscErrorCode ModelApplyBoundaryCondition_Transition(BCList bclist,DM dav,pTatinCtx c,ModelTranspressionCtx *data)
{
  BC_Lithosphere bcdata;
  ...
  if (time >= data->BC_time[0] && time < data->BC_time[1]) {
    ...
    vx = ((0.0-data->v_compression)/(data->BC_time[1]-data->BC_time[0])) * (time-data->BC_time[1]);
    vz = 0.0;
    vxl = -vx; vxr =  vx;
    vzl = -vz; vzr =  vz;
  ...
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMIN_LOC,0,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
  bcdata->v = vzl;
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMIN_LOC,2,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
  bcdata->v = 0.0;
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMIN_LOC,1,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
  
  bcdata->y_lab = data->y_continent[2];
  bcdata->v = vxr;
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMAX_LOC,0,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
  bcdata->v = vzr;
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMAX_LOC,2,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
  bcdata->v = 0.0;
  ierr = DMDABCListTraverse3d(bclist,dav,DMDABCList_IMAX_LOC,1,BCListEvaluator_Lithosphere_transpression,(void*)bcdata);CHKERRQ(ierr);
...
static PetscErrorCode ModelApplyBoundaryCondition_ObliqueCompression(BCList bclist,DM dav,pTatinCtx c,ModelTranspressionCtx *data)
{
  BC_Lithosphere bcdata;
...
PetscErrorCode ModelApplyBoundaryConditionVelocity_Transpression(BCList bclist,DM dav,pTatinCtx c,ModelTranspressionCtx *data)
{
  ...
  */
  ...
  if (time < data->BC_time[0]) {
    ierr = ModelApplyBoundaryCondition_OrthogonalCompression(bclist,dav,c,data);CHKERRQ(ierr);
...
static PetscErrorCode ModelApplyTimeDependantEnergyBCs(pTatinCtx c,ModelTranspressionCtx *data)
{
  PhysCompEnergyFV energy;
  PetscReal        val_T;
  PetscInt         l;
...
PetscErrorCode ModelApplyBoundaryCondition_Transpression(pTatinCtx c,void *ctx)
{
  ModelTranspressionCtx *data;
  PhysCompStokes   stokes;
  DM               stokes_pack,dav,dap;
  ...
  ierr = ModelApplyBoundaryConditionVelocity_Transpression(stokes->u_bclist,dav,c,data);CHKERRQ(ierr);
...
PetscErrorCode ModelApplyBoundaryConditionMG_Transpression(PetscInt nl,BCList bclist[],DM dav[],pTatinCtx c,void *ctx)
{
  ModelTranspressionCtx *data;
  PetscInt         n;
  PetscErrorCode   ierr;
  ...
  for (n=0; n<nl; n++) {
    ierr = ModelApplyBoundaryConditionVelocity_Transpression(bclist[n],dav[n],c,data);CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}

PetscErrorCode ModelApplyMaterialBoundaryCondition_Transpression(pTatinCtx c,void *ctx)
{
  ModelTranspressionCtx *data;
  PhysCompStokes   stokes;
  DM               stokes_pack,dav,dap;
  ...
  for (f=0; f<n_face_list; f++) {
    ...
    /* assign coords */
    epsilon = 1.0e-6;
    ierr = SwarmMPntStd_CoordAssignment_FaceLatticeLayout3d_epsilon_transpression(dav,Nxp,perturb,epsilon,face_list[f],material_point_face_db);CHKERRQ(ierr);
    /* assign values */
...
PetscErrorCode ModelApplyUpdateMeshGeometry_Transpression(pTatinCtx c,Vec X,void *ctx)
{
  ModelTranspressionCtx *data;
  PhysCompStokes   stokes;
  DM               stokes_pack,dav,dap;
...
PetscErrorCode ModelOutput_Transpression(pTatinCtx c,Vec X,const char prefix[],void *ctx)
{
  ModelTranspressionCtx *data;
  PetscBool        active_energy;
  ...
  {
    const MaterialPointVariable mp_prop_list[] = { MPV_region, MPV_viscosity, MPV_density, MPV_plastic_strain, MPV_yield_indicator}; //MPV_viscous_strain,
...
PetscErrorCode ModelDestroy_Transpression(pTatinCtx c,void *ctx)
{
  ModelTranspressionCtx *data;
  PetscErrorCode   ierr;
...
PetscErrorCode pTatinModelRegister_Transpression(void)
{
  ModelTranspressionCtx *data;
  pTatinModel      m;
  PetscErrorCode   ierr;
  ...
  ierr = PetscMalloc(sizeof(ModelTranspressionCtx),&data);CHKERRQ(ierr);
  ierr = PetscMemzero(data,sizeof(ModelTranspressionCtx));CHKERRQ(ierr);

  /* register user model */
  ierr = pTatinModelCreate(&m);CHKERRQ(ierr);

  /* Set name, model select via -ptatin_model NAME */
  ierr = pTatinModelSetName(m,"transpression");CHKERRQ(ierr);

  /* Set model data */
  ierr = pTatinModelSetUserData(m,data);CHKERRQ(ierr);

  /* Set function pointers */
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_INIT,                  (void (*)(void))ModelInitialize_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_INIT_MESH_GEOM,  (void (*)(void))ModelApplyInitialMeshGeometry_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_INIT_MAT_GEOM,   (void (*)(void))ModelApplyInitialMaterialGeometry_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_INIT_STOKES_VARIABLE_MARKERS,(void (*)(void))ModelApplyInitialStokesVariableMarkers_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_INIT_SOLUTION,   (void (*)(void))ModelApplyInitialSolution_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_BC,              (void (*)(void))ModelApplyBoundaryCondition_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_BCMG,            (void (*)(void))ModelApplyBoundaryConditionMG_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_MAT_BC,          (void (*)(void))ModelApplyMaterialBoundaryCondition_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_APPLY_UPDATE_MESH_GEOM,(void (*)(void))ModelApplyUpdateMeshGeometry_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_OUTPUT,                (void (*)(void))ModelOutput_Transpression);CHKERRQ(ierr);
  ierr = pTatinModelSetFunctionPointer(m,PTATIN_MODEL_DESTROY,               (void (*)(void))ModelDestroy_Transpression);CHKERRQ(ierr);

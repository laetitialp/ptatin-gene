...
PetscErrorCode BCListDestroy(BCList *list)
{
  ...
  PetscFunctionBegin;
  {
    ...
    //PetscPrintf(PETSC_COMM_WORLD,"BCList(global): only %" PetscInt_FMT " of %" PetscInt_FMT " entries need to be stored \n", cnt, ll->L );
    cnt = 0;
    for (n=0; n<ll->L_local; n++) {
      ierr = BCListIsDirichlet(ll->dofidx_local[n],&isdir);CHKERRQ(ierr);
      if (isdir==PETSC_TRUE) { cnt++; }
    }
    //PetscPrintf(PETSC_COMM_WORLD,"BCList(local): only %" PetscInt_FMT " of %" PetscInt_FMT " entries need to be stored \n", cnt, ll->L );
  }

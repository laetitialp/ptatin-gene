/*
  ...
  on DESKTOP-S1BGS4R, at 2023-05-23 11:09:43.736350 by tony
*/
...
const int QPntVolCoefStokes_nmembers = 5;
...
MPI_Datatype MPI_QPNTVOLCOEFSTOKES;
...
{
  memcpy( &point->gravity_vector[0], data, sizeof(double)*3 );
}
...
void QPntVolCoefStokesView(QPntVolCoefStokes *point)
{
  ...
  {
    double *data;
    QPntVolCoefStokesGetField_gravity_vector(point,&data);
    printf("field: gravity_vector[0] = %1.6e; [size %zu; type double; variable_name gravity_vector]\n",data[0], QPntVolCoefStokes_member_sizes[4] );
    printf("field: gravity_vector[1] = %1.6e; [size %zu; type double; variable_name gravity_vector]\n",data[1], QPntVolCoefStokes_member_sizes[4] );
    printf("field: gravity_vector[2] = %1.6e; [size %zu; type double; variable_name gravity_vector]\n",data[2], QPntVolCoefStokes_member_sizes[4] );
  }
}
...
void QPntVolCoefStokesVTKWriteBinaryAppendedHeaderAllFields(FILE *vtk_fp,int *offset,const int N,const QPntVolCoefStokes points[]) 
{
  ...
  *offset = *offset + sizeof(int) + N * sizeof(double);
  ...
  /* Warning: swarm_class_generator.py is ignoring multi-component field gravity_vector[] */
...
/* ===================================== */
/* MPI data type for QPntVolCoefStokes */
/* ===================================== */
int QPntVolCoefStokesCreateMPIDataType(MPI_Datatype *ptype)
{
  ...
  int blocklens[] = { 1 , 1 , 3 , 1 , 3 };
  MPI_Aint loc[6];
  MPI_Aint disp[5];
  QPntVolCoefStokes dummy;
  int i,ierr;

  ierr = MPI_Get_address(&dummy,&loc[0]);
  ierr = MPI_Get_address(&dummy.eta,&loc[1]);
  ierr = MPI_Get_address(&dummy.rho,&loc[2]);
  ierr = MPI_Get_address(&dummy.Fu,&loc[3]);
  ierr = MPI_Get_address(&dummy.Fp,&loc[4]);
  ierr = MPI_Get_address(&dummy.gravity_vector,&loc[5]);
